...
| 🤖 **Agent tùy chỉnh** | Sidebar → **🤖 Agent của tôi → ＋** → đặt tên + emoji + **hướng dẫn/tính cách** → LUMINA lưu thành "trợ lý riêng". Bấm chọn agent → thanh agent hiện trên ô nhập, mọi câu chat làm theo hướng dẫn đó (vẫn giữ an toàn/không lộ model). Đây là **"môi trường tạo agent" khả thi** (như Custom GPTs) — KHÔNG phải agent tự chạy tool/code | Lưu theo tài khoản; hướng dẫn chèn vào system prompt |
