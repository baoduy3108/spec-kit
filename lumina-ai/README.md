...
2. 🆓 **Tầng thường — miễn phí**: hết lượt cao cấp, LUMINA **tự tụt xuống** Gemini (free) → Groq (free, phục vụ **Llama 4**) → GitHub Models (free) → OpenRouter (free, phục vụ **Qwen/Llama**) → DeepSeek (rẻ) → **Mistral** (free tier) → **Kimi K3** (Moonshot — MoE 2.8T, ngữ cảnh ~1M, qua API tương thích OpenAI) → **Ollama local (nhiều model)** → OpenAI. Người dùng vẫn chat tiếp bình thường, **không hề biết đã đổi bộ não**.
