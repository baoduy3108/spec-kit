...
| ⚙️ **Lumina Forge** | Bấm nút **⚙️ Lumina Forge** → LUMINA phân tích/thiết kế/triển khai theo quy trình 6 giai đoạn nghiêm ngặt (SPEC → Design Review → Implementation → Validation → Review → Handover); nếu chưa có code/tài liệu trong hội thoại, LUMINA sẽ hỏi bạn dán vào hoặc dùng 📎/dán link trước khi phân tích | Bộ não có tìm kiếm (Gemini/Claude) |
| 💻 **Code Chuyên Sâu** | Bấm nút **💻 Code Chuyên Sâu** → LUMINA trả lời như kỹ sư cấp cao: chọn ngôn ngữ/framework phù hợp, trình bày kiến trúc trước khi viết code, nêu rủi ro bảo mật + cách kiểm thử, kèm Dockerfile/CI-CD nếu liên quan. Hỗ trợ Python/TypeScript/JavaScript/Rust/Go/Java/C#/C++/SQL/Bash | Bộ não có tìm kiếm (Gemini/Claude) |
