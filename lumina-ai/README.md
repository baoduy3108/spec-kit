...
| 🌍 **Đa ngôn ngữ** | Bộ chọn ngôn ngữ ở sidebar: **Tiếng Việt / English / 中文** (giao diện dịch ngay, nhớ lựa chọn). Bộ não vẫn tự trả lời theo ngôn ngữ bạn gõ | Giao diện i18n sẵn; khớp kỹ năng hoạt động cho cả truy vấn tiếng Anh |
...
| 🤖 **Agent tùy chỉnh** | Sidebar → **🤖 Agent của tôi**: có sẵn **⚙️ Lumina Forge** (kỹ sư cấp cao + tự áp dụng **toàn bộ 741 kỹ năng**), và **＋** để tự tạo agent riêng (tên + emoji + hướng dẫn/tính cách). Chọn agent → thanh agent hiện trên ô nhập, mọi câu chat làm theo hướng dẫn đó **và cũng tự áp dụng thư viện kỹ năng** (không lộ model). Đây là **"môi trường tạo agent" khả thi** (như Custom GPTs) — KHÔNG phải agent tự chạy tool/code | Lưu theo tài khoản; hướng dẫn + kỹ năng khớp chèn vào system prompt |
