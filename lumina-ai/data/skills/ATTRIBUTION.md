...
**Nhóm ngôn ngữ/framework + bảo mật + business** (`rust-ownership`, `go-concurrency`,
`typescript-advanced-types`, `nextjs-patterns`, `svelte-patterns`, `express-and-node-apis`,
`threat-modeling`, `owasp-top-10`, `security-headers`, `how-tor-works`,
`how-encryption-at-rest-works`, `saas-metrics`, `cohort-and-funnel-analysis`,
`technical-writing`): LUMINA tự biên soạn theo thông lệ ngành + tài liệu chính thức của từng
ngôn ngữ/framework (Rust Book, Go blog, TypeScript/Next.js/Svelte docs), khung OWASP Top 10 &
STRIDE (kiến thức bảo mật phổ thông), khung Diátaxis cho technical writing, và các định nghĩa
chuẩn của metric SaaS/cohort/funnel — nội dung là best-practice phổ biến, không sao chép nguyên
văn từ nguồn nào.
