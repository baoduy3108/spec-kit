...
description: Quick global settings — currency, language, region, units, theme — belong in a persistent, low-profile location such as a header utility strip or footer. They are frequent but not primary, so they use small muted typography and stay out of the main content hierarchy. Use when designing global selectors, locale switchers, or user preference controls that apply across the whole product.
category: design
keywords_vi: bộ chọn ngôn ngữ, chọn tiền tệ, locale switcher, thanh cài đặt toàn cục, đặt currency selector, chọn khu vực đơn vị, header utility bar
---
...
- **Font size: 13–14px** — deliberately smaller than body text
...
- On selection, apply immediately and confirm with a brief status update if the change has a visible effect
