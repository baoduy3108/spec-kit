// A lantern's beam board — it does two jobs.
//
//   as a LOCK  (4x1, puzzle x arcade): light the lantern cell to unseal it,
//              while the dark keeps rising in the shaft behind you;
//   as an ENGINE (2x4, puzzle x idle): the same board, kept afterwards, and
//              how well you route the beam through its collectors is the
//              lantern's yield per second — forever, including offline.
//
// So the puzzle is not a gate bolted onto an idle game. The puzzle *is* the
// production curve: you do not buy a better lantern, you route it better.
//
// Same beam engine and same exhaustive solver as the standalone puzzle game,
// on a smaller grid — a lock should be thirty seconds of thought, not five
// minutes. Every lock is still proven solvable before it is handed over.
//
...
function attemptGenerate(rng, difficulty) {
  ...
  const bounces = Math.min(4, 1 + Math.floor(difficulty));
  const lanternCount = 1;
  ...
  const decoys = Math.min(2, Math.floor(difficulty / 2));
...
// --- the board as a machine ----------------------------------------------

export const COLLECTOR_WORTH = 0.55;
export const LENGTH_WORTH = 0.012;

/**
 * What this board earns, as a multiplier on the lantern's output.
 * Collectors (the lantern cells) are worth most; a longer beam is worth a
 * little, so there is always a slightly better routing to find.
 */
export function scoreBoard(grid, emitter) {
  const result = trace(grid, emitter);
  return 1 + result.litLanterns * COLLECTOR_WORTH + result.segments.length * LENGTH_WORTH;
}

/**
 * The best score this board can reach, found the same exhaustive way the
 * solver proves a lock pickable. Being able to show "2.4 of a possible 3.1"
 * is what makes tuning a lantern feel like a puzzle rather than a slider.
 */
export function bestScore(grid, emitter, maxMovable = 14) {
  const movable = [];
  grid.forEach((cell, i) => {
    if (!cell.fixed && ROTATABLE.has(cell.kind)) movable.push(i);
  });
  if (movable.length > maxMovable) return { score: scoreBoard(grid, emitter), rotations: null };

  const work = cloneGrid(grid);
  let best = { score: -Infinity, rotations: null };
  for (let mask = 0; mask < 1 << movable.length; mask++) {
    for (let b = 0; b < movable.length; b++) work[movable[b]].rot = (mask >> b) & 1;
    const score = scoreBoard(work, emitter);
    if (score > best.score) best = { score, rotations: movable.map((i) => work[i].rot) };
  }
  return best;
}

/** A board with somewhere for the light to go: two or three collectors. */
export function generateEngine(rng, difficulty = 2) {
  const puzzle = generate(rng, { difficulty });
  if (!puzzle) return null;
  const empties = [];
  puzzle.grid.forEach((c, i) => {
    if (c.kind === KIND.EMPTY) empties.push(i);
  });
  const extra = Math.min(2, Math.max(1, Math.floor(difficulty / 2)));
  for (let k = 0; k < extra && empties.length; k++) {
    const at = empties.splice(rng.int(0, empties.length - 1), 1)[0];
    puzzle.grid[at] = { kind: KIND.LANTERN, rot: 0, fixed: true };
  }
  const best = bestScore(puzzle.grid, puzzle.emitter);
  return { ...puzzle, best: best.score };
}
