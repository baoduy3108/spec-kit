// Bundles the game into one self-contained HTML file (dist/lumenfall.html)
...
/** Names a module *declares* at top level — these share the bundle scope. */
...
function loadModule(file) {
  ...
  modules.set(file, {
    ...
    deps,
    namespaces,
    exports: exportedNames(raw),
    declares: declaredNames(code),
  });
...
// same top-level name produce a file that only fails at runtime. Catch it here.
const declared = new Map();
for (const file of order) {
  const mod = modules.get(file);
  const names = new Set([...mod.declares, ...mod.namespaces.map((ns) => ns.name)]);
  for (const name of names) {
    if (declared.has(name)) {
      throw new Error(
        `duplicate top-level name "${name}" in ${file.slice(root.length + 1)} and ` +
          `${declared.get(name).slice(root.length + 1)} — rename one or share it`,
      );
    }
...
const out = join(root, 'dist/lumenfall.html');
