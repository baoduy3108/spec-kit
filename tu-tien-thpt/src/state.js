...
TD.bangCuaThe = function (the) {
  ...
  (than.match(/<table[\s\S]*?<\/table>/g) || []).forEach(b => {
    ...
    /* Bảng NHIỀU CỘT có hàng tiêu đề: mỗi ô dữ liệu là một dữ kiện hỏi được.
       Thẻ "Polymer – trùng hợp" là bảng bốn cột Monomer · Polymer · Loại phản
       ứng · Ứng dụng — trước đây bị bỏ qua sạch vì code chỉ nhận bảng hai cột. */
    const hang = b.match(/<tr[\s\S]*?<\/tr>/g) || [];
    const dau = hang[0] && (hang[0].match(/<th[^>]*>[\s\S]*?<\/th>/g) || []);
    if (!dau || dau.length < 3) return;
    const sach = t => t.replace(/<t[dh][^>]*>|<\/t[dh]>/g, '').replace(/<\/?(b|i|em|strong|u)>/g, '').trim();
    const tenCot = dau.map(t => sach(t).replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim());
    hang.slice(1).forEach(tr => {
      const o = tr.match(/<td[^>]*>[\s\S]*?<\/td>/g) || [];
      if (o.length !== tenCot.length) return;
      const v = o.map(sach);
      ...
      for (let j = 1; j < v.length; j++) {
        ...
        ra.push({ nhan: `${tenCot[j]} của ${tenCot[0] ? tenCot[0] + ' ' : ''}${khoa}`,
          noi: v[j], tran: tran2, chat: TD.chatTrongO(tran2),
          /* nhiễu phải lấy trong CÙNG CỘT, không thì hỏi "loại phản ứng" mà
             bày ra tên polymer — lộ đáp án ngay */
          _nhom: 'cot' + j, _cot: true, _khoa: khoa, _tenKhoa: tenCot[0], _tenCot: tenCot[j] });
      }
    });
  });
...
/* Bóc các dòng "Nhãn: nội dung" trong thân thẻ thành cặp hỏi được.
   Lọc chặt để không bẻ nhầm giữa một câu văn: vế trái phải NGẮN, không kết
   thúc bằng dấu chấm, vế phải phải đủ dài để thành một phương án tử tế. */
TD.dongVanCuaThe = function (than) {
  ...
  String(than).split(/<br\s*\/?>|<\/p>|\n/).forEach(d => {
    ...
    if (van.length < 12 || van.length > 170) return;
    const i = van.indexOf(':');
    if (i < 1) return;                       /* nhãn một ký tự như "γ:" vẫn hợp lệ */
    const nhan = van.slice(0, i).trim();
    const tran = van.slice(i + 1).trim().replace(/[.;]$/, '');
    if (nhan.length > 45 || /[.!?]$/.test(nhan) || nhan.indexOf(':') >= 0) return;
    if (tran.length < 10 || tran.length > 120 || tran.indexOf(':') >= 0) return;
    /* nhãn mà đã chứa luôn câu trả lời thì hỏi cũng bằng thừa */
    if (TD.khongDau(tran).indexOf(TD.khongDau(nhan)) >= 0) return;
    ra.push({ nhan: nhan, noi: tran, tran: tran, chat: TD.chatTrongO(tran), _nhom: 'van', _van: true });
  });
...
TD.cauBangThe = function (mon, the, cd, seed, canBaoNhieu) {
  ...
  const dungDo = (a, b) => {
    ...
    /* So theo CHUỖI có thứ tự, không phải tập từ. Tập từ coi "Chắc 3 ý, đoán 1"
       và "Chắc 1 ý, đoán 3" là một vì cùng bộ chữ — mà đó lại đúng là cặp bẫy
       đảo số cần giữ. */
    const cA = ' ' + tach(a).join(' ') + ' ', cB = ' ' + tach(b).join(' ') + ' ';
    if (cA.trim() === '' || cB.trim() === '') return true;
  ...
  dong.forEach(d => {
    const cungNhom = dong.filter(z => z !== d && z._nhom === d._nhom);
    const khac = dong.filter(z => z !== d);
    /* Dòng bóc từ cột phụ hoặc từ câu văn thì phương án nhiễu CHỈ được lấy
       trong chính thẻ, cùng cột hoặc cùng kiểu. Mượn thẻ khác là ra câu hỏi
       phóng xạ mà phương án là công thức cường độ âm — nhìn phát loại luôn. */
    const trongThe = d._nhom !== 'bang';
    const cungKieu = z => (z.chat.length > 0) === (d.chat.length > 0);
    const hoiNoi = d._cot
      ? `Theo bảng trong thẻ <b>«${ten}»</b>, ${d._tenKhoa ? d._tenKhoa + ' ' : ''}<b>${d._khoa}</b> `
        + `có <b>${d._tenCot}</b> là gì?`
      : `Theo thẻ <b>«${ten}»</b>, <b>${d.nhan}</b> ứng với nội dung nào sau đây?`;
  ...
  /* ⑦ + ⑧ — "ghép nào ĐÚNG / ghép nào SAI".
     Đây là dạng cứu cho những thẻ chỉ có ba dòng: hỏi "mục X ứng với nội dung
     nào" thì cần ba phương án nhiễu mà chỉ có hai dòng còn lại, nên trước đây
     bảng ba dòng ra ĐÚNG 0 CÂU. Ghép chéo nhãn của dòng này với nội dung của
     dòng kia thì ba dòng đã cho sáu cặp sai — thừa sức dựng câu, mà vẫn đúng
     dạng "phát biểu nào sau đây đúng" của đề thật. */
  /* Dấu nối là ⇒ chứ không phải — : chính nội dung trong thẻ đã dùng "—" để
     chú thích ("A giảm 4, Z giảm 2 — đâm xuyên yếu"), dùng chung một dấu thì
  ...
  /* Câu ghép dùng TOÀN BỘ bảng chứ không tách theo cột: nhãn hiển thị đã kèm
     ...
     như "Carboxylic acid" — mới dựng nổi câu. */
  [dong.filter(ghepDuoc)].forEach(nh => {
    if (nh.length < 3) return;
    const capSai = [];
    /* Thẻ có thể có HAI DÒNG CÙNG NHÃN (thẻ cấp số có "Ba số liên tiếp" cho cả
       cấp số cộng lẫn cấp số nhân). Ghép chéo khi đó lại ra đúng chuỗi của
       dòng kia, thành hai phương án y hệt nhau trong một câu. */
    const dungTatCa = {};
    nh.forEach(d => { dungTatCa[ghep(d, d.noi)] = 1; });
    nh.forEach(a => nh.forEach(b => {
      if (a === b) return;
      if (dungTatCa[ghep(a, b.noi)]) return;
      /* Hai nội dung quá giống nhau thì ghép chéo chưa chắc đã sai: "chỉ ③
         Na₂CO₃/Na₃PO₄" gán cho dòng ghi "③ Na₂CO₃/Na₃PO₄ (hoặc trao đổi ion)"
         vẫn đúng trên thực tế. Chỉ ghép chéo khi hai nội dung khác hẳn nhau. */
      if (dungDo(a.tran, b.tran)) return;
      capSai.push({ t: ghep(a, b.noi), a: a, b: b });
    }));
    if (capSai.length < 3) return;
    /* mỗi dòng một câu "ghép nào ĐÚNG" */
    nh.forEach(d => {
      /* Chỉ cần loại cặp có NHÃN trùng đáp án — hai phương án cùng nhãn là lộ.
         Cặp mượn NỘI DUNG của d gán sang nhãn khác vẫn là nhiễu tốt, và với
         bảng ba dòng thì đó chính là chỗ kiếm đủ ba phương án. */
      const s = TD.xaoR(R, capSai.filter(z => z.a !== d)).slice(0, 3);
      if (s.length < 3) return;
      const dung = ghep(d, d.noi);
      const opts = TD.xaoR(R, [dung].concat(s.map(z => z.t)));
    ...
    /* câu "ghép nào SAI" cần ba phương án ĐÚNG nguyên văn, tức bốn dòng trở lên */
    if (nh.length < 4) return;
    TD.xaoR(R, capSai.slice()).slice(0, nh.length).forEach(z => {
      const s = TD.xaoR(R, nh.filter(d => d !== z.a && d !== z.b)).slice(0, 3);
      if (s.length < 3) return;
      const opts = TD.xaoR(R, [z.t].concat(s.map(d => ghep(d, d.noi))));
