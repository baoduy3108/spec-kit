...
TD.cauBangThe = function (mon, the, cd, seed, canBaoNhieu) {
  ...
  /* Bốn phương án phải là bốn chuỗi KHÁC NHAU. Thẻ có hai dòng cùng nhãn
     ("∫ab" xuất hiện ở nhiều tính chất tích phân) thì ghép chéo dễ ra hai
     phương án y hệt nhau trong một câu. */
  const bonKhac = opts => {
    const da = {};
    return opts.length === 4 && opts.every(o => {
      const k = String(o).replace(/<[^>]+>/g, '').trim();
      if (!k || da[k]) return false;
      da[k] = 1; return true;
    });
  };
  ...
  dong.forEach(d => {
    ...
    const hoiNoi = d._cot
      ...
      : d._dau === '=' ? `Theo thẻ <b>«${ten}»</b>, <b>${d.nhan}</b> bằng biểu thức nào sau đây?`
