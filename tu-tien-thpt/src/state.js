  ...
  (than.match(/<table[\s\S]*?<\/table>/g) || []).forEach(b => {
    ...
    hang.slice(1).forEach(tr => {
      ...
      for (let j = 1; j < v.length; j++) {
        ...
        ra.push({ nhan: `${tenCot[j]} của ${tenCot[0].toLowerCase()} ${khoa}`,
...
TD.cauBangThe = function (mon, the, cd, seed, canBaoNhieu) {
  ...
  const dungDo = (a, b) => {
    ...
    /* Chỉ chặn khi phương án này CHỨA TRỌN phương án kia ("Cu, Ag" nằm gọn
       trong "Cu, Ag (mọi KL sau H)" là hai đáp án cùng đúng). Trước đây còn
       chặn theo tỉ lệ trùng quá nửa, nhưng thế thì loại luôn cặp nhiễu tốt
       nhất: "Z tăng 1" và "Z giảm 1" chỉ khác nhau đúng một từ. */
  ...
  dong.forEach(d => {
    ...
    const hoiNoi = d._cot
      ? `Theo bảng trong thẻ <b>«${ten}»</b>, ${d._tenKhoa.toLowerCase()} <b>${d._khoa}</b> `
        + `có <b>${d._tenCot.toLowerCase()}</b> là gì?`
