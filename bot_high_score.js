...
async function highScoreBot() {
  ...
  try {
    console.log('💰 HIGH SCORE BOT - Aggressive Scoring\n');
    console.log('Strategy: Maximize points per clear - 2-3 lines = JACKPOT\n');
    ...
    while (moveCount < maxMoves) {
      const result = await page.evaluate(async () => {
        ...
        // HIGH SCORE STRATEGY - MASSIVE BONUSES FOR MULTI-CLEARS
          ...
          for (let r = 0; r <= 8 - piece.h; r++) {
            for (let c = 0; c <= 8 - piece.w; c++) {
              if (window.__gameAPI.canPlace(piece, r, c)) {
                ...
                // ========== AGGRESSIVE SCORING - TĂNG ĐÁP ĐỦ ==========
                if (totalClears === 0) {
                  score += 10;
                } else if (totalClears === 1) {
                  score += 400;  // Tăng từ 200 -> 400
                } else if (totalClears === 2) {
                  // DUAL-LINE = JACKPOT x5 (từ 2000 -> 10000)
                  score += 10000;
                } else if (totalClears === 3) {
                  // TRIPLE CLEAR = MEGA JACKPOT (từ 8000 -> 50000)
                  score += 50000;
                } else if (totalClears >= 4) {
                  // 4+ CLEARS = ABSOLUTELY INSANE
                  score += 100000 + (totalClears * 20000);
                }
    ...
    console.log(`\n${'═'.repeat(70)}`);
    console.log(`💰 HIGH SCORE BOT FINAL SCORE: ${final.score} points`);
    console.log(`📊 Total moves: ${moveCount}`);
    console.log(`💡 Strategy: Aggressive scoring - 2 lines = 10k, 3 lines = 50k, 4+ = 100k+`);
...
highScoreBot().catch(console.error);
